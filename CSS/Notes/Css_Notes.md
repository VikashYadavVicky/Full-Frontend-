...
# 🌐 Useful Web Resources for CSS, Design, and Development
...
CSS selectors are patterns used to select and style the elements on a webpage. They define **which HTML elements** a set of CSS rules will be applied to.
...
Each CSS rule gets a specificity value (A,B,C,D):
...
➡️ **Result**: Inline style > ID selector > class selector > element selector and The <p> will appear purple, because inline style wins.
...
CSS provides multiple ways to apply colors to HTML elements, including text, borders, backgrounds, and more.
...
| `vmax` | Larger of `vw` or `vh`            | Same, but takes the bigger value.              |
...
*Explanation: This centers the `.center-box` horizontally with a top/bottom margin of 20px.*
...
*Explanation: A dashed blue border of 3px surrounds the box.*
...
> **Note:** The correct order for pseudo-classes: **LVHA** → `:link`, `:visited`, `:hover`, `:active`
...
CSS `position` property defines how an element is positioned in a document. It works with `top`, `right`, `bottom`, and `left` properties to move elements.
...
* Stays in normal flow but shifted visually.
...
⚠️ `z-index` only works on positioned elements (`position: relative`, `absolute`, `fixed`, or `sticky`).
...
- Use `z-index` incrementally: 10, 100, 1000 to leave room for inserts.
...
* `overflow` handles extra content visibility.
...
To use grid, define a container with `display: grid` or `display: inline-grid`:
...
## 🌄 Example 1: Responsive Image with `object-fit`
...
## 🎨 Visual Hover/Focus Mockup Example
...
## 📊 Common Variable Examples Table
...
| `calc()`                                       | Math        | Calculates values dynamically                           | `width: calc(100% - 50px)`                         |
| `clamp()`                                      | Math        | Restricts a value between a defined min, ideal, and max | `font-size: clamp(1rem, 2vw, 2rem)`                |
| `min()`                                        | Math        | Returns the smallest value                              | `width: min(500px, 80%)`                           |
| `max()`                                        | Math        | Returns the largest value                               | `height: max(50vh, 300px)`                         |
| `rgb() / rgba()`                               | Colors      | Defines colors with red, green, blue (+ alpha)          | `background: rgba(255, 0, 0, 0.5)`                 |
| `hsl() / hsla()`                               | Colors      | Defines colors using hue, saturation, lightness         | `color: hsl(240, 100%, 50%)`                       |
| `url()`                                        | URLs        | Inserts a file path (image, font, etc.)                 | `background-image: url('img/bg.jpg')`              |
| `attr()`                                       | Content     | Retrieves HTML attribute value (in pseudo-elements)     | `content: attr(data-label)`                        |
| `linear-gradient()`                            | Gradient    | Creates a linear gradient                               | `background: linear-gradient(to right, red, blue)` |
| `radial-gradient()`                            | Gradient    | Creates a radial gradient                               | `background: radial-gradient(circle, red, blue)`   |
| `repeating-linear-gradient()`                  | Gradient    | Repeats linear gradients                                | `background: repeating-linear-gradient(...)`       |
| `repeating-radial-gradient()`                  | Gradient    | Repeats radial gradients                                | `background: repeating-radial-gradient(...)`       |
| `cubic-bezier()`                               | Animation   | Custom timing function for animation                    | `animation-timing-function: cubic-bezier(...)`     |
| `steps()`                                      | Animation   | Creates stepped timing functions                        | `animation-timing-function: steps(4)`              |
| `rotate()`, `scale()`, `translate()`, `skew()` | Transform   | Applies 2D/3D transformations                           | `transform: rotate(45deg)`                         |
| `matrix()`                                     | Transform   | Defines a 2D transformation using matrix values         | `transform: matrix(...)`                           |
| `env()`                                        | Environment | Accesses environment variables (safe-area-inset, etc.)  | `padding: env(safe-area-inset-top)`                |
| `round()`                                      | Math (new)  | Rounds values (under `math` functions in CSS Values 4)  | `width: round(23.7px)` (limited support)           |

## 🔄 Variable & Environment Functions

### `var()`

Used to retrieve the value of a CSS variable.
...
Access environment-specific constants like safe-area insets (used in mobile, iOS).
...
Allows real-time calculation of property values.
...
Maintains responsiveness while keeping values within limits.
...
Choose the smallest or largest of multiple values.
...
Rounds values with specified precision.

```css
width: round(25.7px, 0.1px);
```
...
Use red, green, blue and optional alpha.
...
Hue, Saturation, Lightness based color format.
...
Provides external file location for images, fonts, etc.
...
Retrieves HTML attribute value. Often used in `content:` property of pseudo-elements.
...
## 🌧️ Gradient Functions

### `linear-gradient()`

Creates linear transitions between colors.
...
Spreads from a central point outward.
...
Custom easing curve between animation keyframes.
...
Breaks animation into discrete steps.
...
Defines a matrix-based 2D transform.

```css
transform: matrix(1, 0.3, 0.5, 1, 0, 0);
```
...
CSS animations allow you to animate transitions from one CSS style to another. You can control how elements change styles over time using keyframes and animation properties.

## 📋 Animation Properties Table

| Property                    | Description                                  | Example Value                             |
| --------------------------- | -------------------------------------------- | ----------------------------------------- |
| `@keyframes`                | Defines the animation keyframes (steps)      | `@keyframes slideIn { ... }`              |
| `animation-name`            | Name of the `@keyframes` animation           | `slideIn`                                 |
...
| `animation-delay`           | Delay before the animation starts            | `1s`, `500ms`                             |
| `animation-iteration-count` | Number of times the animation repeats        | `1`, `infinite`, `3`                      |
| `animation-direction`       | Direction of the animation cycle             | `normal`, `reverse`, `alternate`          |
| `animation-fill-mode`       | Styles applied before/after animation        | `none`, `forwards`, `backwards`           |
| `animation-play-state`      | Control if animation is running or paused    | `running`, `paused`                       |
| `animation`                 | Shorthand to define all animation properties | `slideIn 2s ease-in-out 1s 3 normal both` |

## 📌 Property Details

### 🔧 `@keyframes`

Defines the keyframes that describe the animation.
...
Sets the time the animation takes to complete one cycle.

```css
div {
  animation-duration: 2s; /* or 2000ms */
}
```
...
```css
div {
  animation-timing-function: ease; /* ease, linear, ease-in, ease-out, cubic-bezier() */
}
```
...
| `steps(n)`       | Discrete jumps in steps                 |
...
Direction in which the animation runs.
...
| Value       | Description                                       |
| ----------- | ------------------------------------------------- |
| `none`      | No styles are retained (default)                  |
| `forwards`  | Keep styles from the last keyframe                |
| `backwards` | Apply styles from the first keyframe before start |
...
Combines all properties into one declaration.

```css
div {
  animation: slideIn 2s ease-in-out 1s 3 alternate both;
}
```
...
## 🚀 CSS Transform – All Properties & Values

CSS `transform` allows you to apply 2D or 3D transformations to elements.

### 🔩 Transform Properties Table

| Property              | Description                          | Example                 |
| --------------------- | ------------------------------------ | ----------------------- |
| `transform`           | Applies 2D or 3D transformation      | `transform: scale(1.5)` |
| `transform-origin`    | Defines origin of transformation     | `top left`, `50% 50%`   |
| `transform-style`     | Specifies flat or 3D child rendering | `flat`, `preserve-3d`   |
| `perspective`         | Defines 3D perspective depth         | `600px`                 |
| `perspective-origin`  | Origin of the perspective            | `center`, `left top`    |
| `backface-visibility` | Hides/show back of transformed item  | `hidden`, `visible`     |

### 🔧 `transform` Functions

| Function         | Description                              |
| ---------------- | ---------------------------------------- |
| `translateX(x)`  | Moves element on X-axis                  |
| `translateY(y)`  | Moves element on Y-axis                  |
| `translate(x,y)` | Moves on both axes                       |
| `scale(x)`       | Scales element proportionally            |
| `scaleX(x)`      | Scales on X-axis                         |
| `scaleY(y)`      | Scales on Y-axis                         |
| `rotate(deg)`    | Rotates element in 2D                    |
| `rotateX(deg)`   | Rotates in 3D around X-axis              |
| `rotateY(deg)`   | Rotates in 3D around Y-axis              |
| `rotateZ(deg)`   | Rotates in 3D around Z-axis              |
| `skewX(deg)`     | Skews on X-axis                          |
| `skewY(deg)`     | Skews on Y-axis                          |
| `matrix()`       | Applies complex 2D transformation matrix |
| `matrix3d()`     | Applies complex 3D transformation matrix |
...
Use `transform-style: preserve-3d;` and `perspective` to give depth to child elements.

## 🧰 Practical Animation + Transform Combos
...
.element {
  animation: slideScale 0.6s ease-out forwards;
}
```

## 🎨 Advanced UI Micro-Interaction Examples
